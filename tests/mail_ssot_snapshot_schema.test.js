...
test("sanitizeSnapshotV1 preserves supported connector ids and strips secrets", () => {
  ...
  assert.equal(snapshot.connectorRefs.length, 2);
  assert.equal(snapshot.connectorRefs[0].connectorId, "mailbox");
  assert.equal(snapshot.connectorRefs[1].connectorId, "outlook");
  assert.equal(Object.prototype.hasOwnProperty.call(snapshot.connectorRefs[0], "apiKey"), false);
  assert.equal(Object.prototype.hasOwnProperty.call(snapshot.connectorRefs[1], "accessToken"), false);
