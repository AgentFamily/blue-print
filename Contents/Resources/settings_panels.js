...
window.getVaultBrokerSessionToken = function getVaultBrokerSessionToken() {
  if (!vaultSession.unlocked) return "";
  const token = String(vaultSession.sessionToken || "").trim();
  if (!token) return "";
  const expiresAt = Number(vaultSession.sessionExpiresAt || 0);
  if (Number.isFinite(expiresAt) && expiresAt > 0 && Date.now() >= expiresAt) return "";
  return token;
};
...
async function vaultUnlockFromInput() {
  ...
  try {
    const unlockMeta = await vaultRequestJson("/api/vault/unlock", {
      method: "POST",
      body: JSON.stringify({
        ttlMs: 10 * 60 * 1000,
        permissions: ["read:OPENAI_API_KEY"],
        botId: "chat-assistant",
      }),
    });
    vaultSession.sessionToken = String(unlockMeta?.sessionToken || "").trim();
    vaultSession.sessionExpiresAt = Number(unlockMeta?.expiresAt || 0);
    try {
      window.__agentcVaultSessionToken = vaultSession.sessionToken;
    } catch {
      // ignore
    }
    await vaultLoadEntries();
    ...
    vaultSession.sessionExpiresAt = 0;
    try {
      window.__agentcVaultSessionToken = "";
    } catch {
      // ignore
    }
...
async function vaultLock() {
  if (vaultSession.unlocked && vaultSession.passphrase) {
    try {
      await vaultRequestJson("/api/vault/lock", {
        method: "POST",
        body: JSON.stringify({ reason: "manual" }),
      });
    } catch {
      // ignore lock relay failures
    }
  }
  vaultSession.unlocked = false;
  vaultSession.passphrase = "";
...
async function renderVaultPanel() {
  ...
  el("vaultLockBtn")?.addEventListener("click", async () => {
    await vaultLock();
    renderVaultPanel();
  });
