...
const getCoinPackByInternalName = (internalName) => {
  const key = String(internalName || "").trim();
  if (!key) return null;
  return COIN_PRODUCT_PACKS.find((p) => String(p.internal_name || "").trim() === key) || null;
};

module.exports = {
  COIN_PRODUCT_PACKS,
  getCoinPackByInternalName,
};
