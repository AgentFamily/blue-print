...
const { createConnectorOauthState } = require("../oauth_state");
...
const connectorActions = (connectorId, workspaceId) => {
  ...
  return {
    ...
    oauthStartUrl: `/api/connectors/${connectorId}/oauth/start${wsQuery}`,
...
const listConnectorCatalog = ({ actorUserId, workspaceId }) => {
  ...
  return listConnectors().map((connector) => ({
    ...
    oauthStartSupported: Boolean(getConnector(connector.id).supportsOauthStart()),
...
const getConnectorRequirements = ({ connectorId, workspaceId }) => {
  ...
  return {
    ...
    oauthStartSupported: Boolean(connector.supportsOauthStart()),
...
const beginConnectorAuthorization = async ({ connectorId, actorUserId, workspaceId }) => {
  ...
  const connector = getConnector(connectorId);
  if (!connector.supportsOauthStart()) {
    throw new BlueprintError(400, "connector_oauth_not_supported", "Connector does not support interactive OAuth");
  }
  ...
  const out = await connector.beginAuthorize({
    actorUserId,
    workspaceId,
    state,
  });
  if (!out?.redirect) {
    throw new BlueprintError(502, "connector_oauth_failed", "Connector did not return an OAuth redirect");
  }

  audit({
    actorUserId,
    workspaceId,
    action: "connector.oauth_start",
