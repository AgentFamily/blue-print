...
const SUPPORTED_CONNECTOR_IDS = new Set(["mailbox", "outlook"]);
...
const normalizeConnectorRef = (input) => {
  ...
  const rawConnectorId = cleanText(source.connectorId, 80).toLowerCase();
  const connectorId = rawConnectorId || "mailbox";
  if (!workspaceId || !connectionId) return null;
  if (!SUPPORTED_CONNECTOR_IDS.has(connectorId)) return null;
  return {
...
const sanitizeSnapshotV1 = (input) => {
  ...
  if (!connectorRefs.length) {
    throw new BlueprintError(400, "validation", "connectorRefs must include at least one mailbox or outlook connection");
  }
