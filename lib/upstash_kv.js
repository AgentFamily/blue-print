...
const kvSet = async (key, value, { exSeconds, nx = false } = {}) => {
...
const kvSetNX = async (key, value, { exSeconds } = {}) => {
  const json = await kvSet(key, value, { exSeconds, nx: true });
  // Upstash returns null result when NX fails.
  return Boolean(json);
};

const kvDel = async (key) => {
  const json = await kvFetch(["del", key]);
  const n = typeof json?.result === "number" ? json.result : parseInt(String(json?.result ?? ""), 10);
  return Number.isFinite(n) ? n : 0;
};

const kvKeys = async (pattern) => {
  const json = await kvFetch(["keys", pattern]);
  return Array.isArray(json?.result) ? json.result.map((x) => String(x || "")).filter(Boolean) : [];
};
